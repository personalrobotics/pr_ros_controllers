...
using namespace force_gate_controller::interpolation_methods;  // NOLINT
...
TEST(TestTrajectory, initialize_trajectory)
{
  auto clock = rclcpp::Clock(RCL_STEADY_TIME);
  {
    ...
    auto traj = force_gate_controller::Trajectory(empty_msg);

    trajectory_msgs::msg::JointTrajectoryPoint expected_point;
    force_gate_controller::TrajectoryPointConstIter start, end;
...
TEST(TestTrajectory, sample_trajectory_positions)
{
  ...
  auto traj = force_gate_controller::Trajectory(time_now, point_before_msg, full_msg);
...
TEST(TestTrajectory, interpolation_pos_vel)
{
  ...
  auto traj = force_gate_controller::Trajectory();
